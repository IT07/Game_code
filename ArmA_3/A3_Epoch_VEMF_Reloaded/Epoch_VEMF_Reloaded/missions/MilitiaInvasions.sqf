...
// Define _settings
_settings = [["MI"],["maxInvasions","groupCount","groupUnits","distanceCheck","distanceTooClose","distanceMaxPrefered","playerCheck","crateAltitude","useMarker","parachuteCrate","crateVisualMarker","crateMapMarker","useAnnouncements","killTheLights","fixTheLights","lightsRange","canLiftCrate"]] call VEMFr_fnc_getSetting;
...
if (VEMFr_invasCount <= _maxInvasions) then
{
	...
	if (typeName _loc isEqualTo "ARRAY") then
	{
		...
		{
			_newMissionMsg = [[format["Militia have invaded %1%2 near %3", if (_locName isEqualTo "Area") then {"an " } else {""}, _locName, mapGridPosition (_loc select 1)], "NEW INVASION"], ""] spawn VEMFr_fnc_broadCast;
		};
		private["_marker"];
		...
		if _playerNear then
		{
			...
			if (count (_spawned select 0) > 0) then
			{
				...
				if _done then
				{
					...
					if (_useAnnouncements isEqualTo 1) then
					{
						_completeMsg = [[format["Militia invasion of %1 @ %2 defeated!", _locName, mapGridPosition (_loc select 1)], "DEFEATED"], ""] spawn VEMFr_fnc_broadCast;
					};
					// Deal with the 50s
