...
_ok = false;
_enabled = ([["MI"],["placeMines"]] call VEMFr_fnc_getSetting) select 0;
if (_enabled isEqualTo 1) then
{
    ...
    if (count _pos isEqualTo 3) then
    {
        ...
        if (_min > -1) then
        {
            ...
            if (_max > _min) then
            {
                _amount = ([["MI"],["minesAmount"]] call VEMFr_fnc_getSetting) select 0;
                if (_amount > -1) then
                {
                    _minesMode = [[["MI"],["minesMode"]] call VEMFr_fnc_getSetting, 0, 1, [0]] call BIS_fnc_param;
                    ...
                    _mines = [];
                    ["fn_placeMines", 1, format["Placing %1 mines at %2", _amount, _pos]] call VEMFr_fnc_log;
