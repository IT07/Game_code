...
[] spawn
{
    ...
    [] spawn
    {
        _uids = getArray (configFile >> "sc_cfgServerGlobalMessenger" >> "adminUIDs");
        while {true} do
        {
            ...
            {
                {
                    if (isPlayer _x) then
                    {
                        if (side _x isEqualTo independent) then
                        {
                            ...
                            {
                                if not(_x in (uiNamespace getVariable "SC_gmKeysGivenTo")) then
                                {
    ...
    "SC_gmMessage" addPublicVariableEventHandler
    {
        ...
        if not (_header isEqualTo "") then
        {
            if not (_sub isEqualTo "") then
            {
                if not (_moving isEqualTo "") then
                {
