/*
    ...
    Description:
    gets config value of given var from ServerInfoMenu config

	Params:
	method 1:
		_this: STRING - name of SINGLE config value to get from root of sc_cfgServerInfoMenu
	method 2:
		_this select 0: ARRAY of STRINGS - MULTIPLE config values to get from root of sc_cfgServerInfoMenu
...
{
	_cfg = missionConfigFile >> "cfgScarCODE" >> "sc_cfgServerInfoMenu" >> _this;
	call _check;
...
if (typeName _this isEqualTo "ARRAY") then
{
	if (typeName (_this select 0) isEqualTo "ARRAY") then
	{
		...
		{
			_cfg = missionConfigFile >> "cfgScarCODE" >> "sc_cfgServerInfoMenu";
			_path = _cfg;
			{
	...
	if not(typeName (_this select 0) isEqualTo "ARRAY") then
	{
		...
		{
			{
				_cfg = missionConfigFile >> "cfgScarCODE" >> "sc_cfgServerInfoMenu" >> _x;
				call _check;
				_r pushBack _v;
			} forEach _this;
		};
